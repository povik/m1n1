...
class ReplicateWrites:
    def __init__(self, backend, bases):
        self.p = backend
        self.bases = bases

    def write(self, addr, data, width):
        for base in self.bases:
            self.p.write(base + addr, data, width=width)

    def read(self, addr, width):
        raise NotImplementedError("read on ReplicateWrites")

class ReplicateAccesses(ReplicateWrites):
    def read(self, addr, width):
        first = self.p.read(self.bases[0] + addr, width=width)
        for base in self.bases[1:]:
            self.p.read(base + addr, width=width)
        return first
